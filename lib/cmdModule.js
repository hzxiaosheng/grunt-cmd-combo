...
module.exports = function (grunt) {
    function normlizePath(ast, relativePath, rootPath) {
        var walker = new UglifyJS.TreeWalker(function (node, descend) {
            ...
            var string = new UglifyJS.AST_String({
                value: path.resolve(relativePath, modName).replace(path.resolve(rootPath), '').replace(/^(\/|\\+)/, '').replace(/\\+/g,"/")
            });
            node.args.pop();
            node.args.push(string);
            return true; // return true to indicate that the descend nodes should not be walked
        });
    ...
    function moduleWalk(modName, rootPath, process, trace) {
        ...
        deps = [];
        content = grunt.file.read(path.join(rootPath, modName + '.js'));
